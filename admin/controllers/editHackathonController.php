 ...
 if(isset($_POST["h_edit"])){
    ...
    if(isset($_FILES['image'])){
        ...
        if(empty($errors)==true){
            ...
            $hackathon = "SELECT * FROM hackathons WHERE `id` = '$id' LIMIT 1 ";
            $result = $conn->query($hackathon);
            $hackathons = $result->fetch_assoc();
            unlink($hackathons['image']);

            //update the image and the daatabase details
            
           move_uploaded_file($file_tmp,"../../assets/uploads/".$file_name);
           $sql = "UPDATE hackathons SET `name`='$h_name',`start_date`='$h_date_b',`end_date`='$h_date_e',`tags`='$h_tag',`description`='$h_description',`image`='$file_name' WHERE id=2";
           $conn->query($sql);
